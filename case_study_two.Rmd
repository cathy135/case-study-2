...
remove_select_cov$Label = factor(remove_select_cov$Label )  
...
# model with main effect for Subject but no interactions (there is warning)
pca.model = glm(Label ~ ., family = binomial(link="logit"), data=train_pca)

# model with no main effect for Subject but with interactions
pca.model = glm(Label ~ Dim.1+Dim.2+Dim.3+Dim.4+Dim.5+Dim.1:Subject#+Dim.2:Subject
                +Dim.3:Subject#+Dim.4:Subject+Dim.5:Subject
                    , family = binomial(link="logit"), data=train_pca)

# model with random effect for intercept for Subject (no warning)
pca.model = glmer(Label ~ Dim.1+Dim.2+Dim.3+Dim.4+Dim.5+(1|Subject)
                    , family = binomial(link="logit"), data=train_pca)

...
remove_select_cov = ALL_df %>%
  ...
  select(-contains("min")) %>%
  select(-contains("max")) %>%
  select(-contains("ACC_chest_X")) %>%
  select(-contains("ACC_chest_Y")) %>%
  select(-contains("ACC_chest_Z"))
...
scaled_remove_select_cov = scale(remove_select_cov_nsubj, center=TRUE, scale = FALSE)
...
```{r run-wrist-model}
smp_size <- floor(0.75 * nrow(wrist_only))
train_ind <- sample(seq_len(nrow(wrist_only)), size = smp_size)
train_wrist <- wrist_only[train_ind, ]
test_wrist <- wrist_only[-train_ind, ]

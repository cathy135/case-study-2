...
```{r}
# library(plotROC)
# ggplot(combocvoutput, aes(d=actual_values, m = fitted_probs)) + 
#   geom_roc(n.cuts = 0) + 
#   geom_abline(intercept = 0)
# library(pROC)
# 
# object = roc(as.factor(combocvoutput$actual_values),  as.factor(combocvoutput$predicted_values))
# plot.roc(x= combocvoutput$actual_values, y = combocvoutput$fitted_probs)
# 
# aug <- augment(pca.model.interact, type.predict = "response", 
#                      type.residuals = "response")
#roc(combocvoutput$actual_values,combocvoutput$predicted_values)$auc
```
...
#summary(pca.model.interact)
```
As the variation in motion increases, the fitted probabilities (where stress is 1) increases similarly for all subjects except for S13 and S2. We note for S13 and S2 that as the variability of motion becomes gerater, the odds of stress changes increase by a smaller amount. However, all subject exhibit a similar increasing trend. This behavior can be corroborated by Table [][][] in Appendix [][][].
...
if stress = 1, then those on the right are ppl whose odds of stress increases as they get 

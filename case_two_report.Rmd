...
```{r distrib-vs-label}
ggplot(remove_select_cov, aes(y=EDA_wrist_mean, x=factor(Subject), fill = factor(Label))) + geom_boxplot() +
    labs(x ="State", y = "EDA mean (wrist)", title = "Fig. 2 Distribution of Mean Wrist EDA by State")
```
After examining boxplots of the distribution of the physiological measures by state, it (Appendix A.1), it appears that only the boxplot for `EDA_wrist_mean` shows a difference in distribution between the stress and amusement states. Specifically, the boxplot (Fig. 2) shows that EDA measured on the wrist stress to have higher values during the stress state.

```{r subject-vs-covariates}
varvssubj <- function(var){
  ggplot(remove_select_cov, aes_(y=as.name(var), x=remove_select_cov$Subject, 
                                 fill = remove_select_cov$Label)) +
    geom_boxplot() +
...
`EDA_wrist_mean`, `TEMP_wrist_mean`

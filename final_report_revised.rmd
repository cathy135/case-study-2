...
```{r pca-cont1, echo=F,warning = F}
var <- get_pca_var(res.pca.wrist)
corrplot::corrplot(var$cos2, is.corr=FALSE, tl.cex = 0.5, number.cex= 3/ncol(remove_select_cov_nb), title = "Fig. 3 QoR Plot for Wrist PCA", mar=c(0,0,1,0))
```
...
p1=plot_model(pca.model.interact, type = "pred", terms = c("Dim.1 [all]", "Subject"), ci.lvl = FALSE) +
  ...
  labs(title = "Fig. 5 Marginal Effects of Motion", x = "Motion")
p2=plot_model(pca.model.interact, type = "pred", terms = c("Dim.2 [all]", "Subject"), ci.lvl = FALSE) +
  scale_fill_manual(values = mycolors)  +
  scale_y_continuous(name="Fitted Probabilites", limits=c(0, 1)) +
  labs(title = "Fig. 6 Marginal Effects of Dermal Temp and Activity", x = "Dermal Temp and Activity")
grid.arrange(p1, p2, nrow = 1)
```

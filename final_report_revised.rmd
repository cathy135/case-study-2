...
```{r pca-cont1, fig.width = 4, fig.height = 4, echo=F,warning = F}
# var <- get_pca_var(res.pca.wrist)
# wc = corrplot::corrplot(var$cos2, is.corr=FALSE, tl.cex = 0.5, number.cex= 3/ncol(remove_select_cov_nb), title = "Fig. 3 QoR Plot for Wrist PCA", mar=c(0,0,1,0))
...
```{r pca-cont-combo1,  echo=F, fig.width = 5, fig.height = 5,warning = F}
var <- get_pca_var(res.pca.wrist)
wc = corrplot::corrplot(var$cos2, is.corr=FALSE, tl.cex = 0.5, number.cex= 3/ncol(remove_select_cov_nb), title = "Fig. 3 QoR Plot for Wrist PCA", mar=c(0,0,1,0))

var <- get_pca_var(res.pca)
corrplot::corrplot(var$cos2, is.corr=FALSE, tl.cex = 0.5, number.cex= 3/ncol(remove_select_cov_nb), title = "Fig. 4 QoR Plot for Combined Data PCA", mar=c(0,0,1,0))
...
Note that the $E4$ superscript refers to components derived from the wrist-worn data, while the $comb$ superscript refers to the components derived from the combined data. 

We then investigated the prediction accuracy and f1 scores of each logistic regression model using a 5-fold cross validation approach. We chose to conisder the f1 score, which is the harmonic mean of precision and recall, due to the fact that this metric is more appropriate for settings with class imbalance. In our current dataset, we see that the stressed response occurs 64% of the time, warranting a consideration of f1 in addition to accuracy. In order to ensure that certain subjects were not over or underrepresented in the testing and training sets, we created stratified folds in which each individual’s representation was proportional to that of their representation in the entire dataset. As indicated in Table 1, the combined data model was shown to be superior in both accuracy and f1 score. A naive predictor would always choose 1 (the state that occurs more frequently in the data) and obtain an accuracy of 0.64. Both the combined data model and wrist-only data model have accuracies greater than 0.64. Therefore, we say that our models are useful in making effective predictions. For the wrist-worn and combined data models, sensitivity analysis was conducted by varying both the number of principal components and the choice of window size. Results in A.5 indicate that our models are not strongly sensitive to either. 
...
```{r, echo=F, fig.width = 10, fig.height = 4,warning = F, message= F}
# Define the number of colors you want
nb.cols <- 15
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)
p1=plot_model(pca.model.interact, type = "pred", terms = c("Dim.1 [all]", "Subject"), ci.lvl = FALSE) +
  scale_fill_manual(values = mycolors) +  
  scale_y_continuous(name="Fitted Probabilites", limits=c(0, 1)) +
  labs(title = "Fig. 5 Marginal Effects of Motion", "Motion")
p2=plot_model(pca.model.interact, type = "pred", terms = c("Dim.2 [all]", "Subject"), ci.lvl = FALSE) +
  scale_fill_manual(values = mycolors)  +
  scale_y_continuous(name="Fitted Probabilites", limits=c(0, 1)) +
  labs(title = "Fig. 6 Marginal Effects of Dermal Temp and Activity", "Dermal Temp and Activity")
grid.arrange(p1, p2, nrow = 1)
```


```{r int-3-4,echo=F, fig.width = 10, fig.height = 4,warning = F, message= F}
p3 = plot_model(pca.model.interact, type = "pred", terms = c("Dim.3 [all]", "Subject"), ci.lvl = FALSE) +
  scale_fill_manual(values = mycolors)  +
  scale_y_continuous(name="Fitted Probabilites", limits=c(0, 1)) +
  labs(title = "Fig. 7 Marginal Effects of Heart Rate", x = "Heart Rate")
p4 = plot_model(pca.model.interact, type = "pred", terms = c("Dim.4 [all]", "Subject"), ci.lvl = FALSE) +
  scale_fill_manual(values = mycolors)  +
  scale_y_continuous(name="Fitted Probabilites", limits=c(0, 1)) +
  labs(title = "Fig. 8 Marginal Effects of Neuro-Muscular Activity", x = "Neuro-Muscular Activity")
grid.arrange(p3, p4, nrow = 1)
```


Figures 5 through 8 show the fitted probability of stress against each principal component, with different colors representing different subjects. From figure 5, we see that as the variation in motion (Dim.1) increases, the probability of stress increases similarly for all subjects, except for subjects 13 and 2. We note that for subjects 13 and 2, as the variability of motion increases, the probability of stress increases at a slower rate than other subjects. However, all subjects exhibit an increasing trend. Figure 6 shows that as the variation in mean dermal temperature and activity increases (Dim.2) increases, the probability of stress increases similarly for all subjects except for subjects 2, 7, and 13. For these three subjects, the probabilities decrease. 

Figure 7 shows that as heart rate (Dim.3) increases, the probability of stress decreases similarly for all subjects except for subjects 5, 6, 9, and 17. For these four subjects, the probabilities increase. Figure 8 shows that as neurological and muscular activity (Dim.4) increase, the probability of stress decreases similarly for all subjects barring subjects 2, 6, 7, 10, and 14. For these five subjects, as neuro-muscular activity increases, the probability of stress increases as well. 

These behaviors can be corroborated by the coefficient estimates in A.4. When adding together the main effect of a principal component and its interaction  with a particular subejct, it can be noted that positive values corespond with an increasing trend in the above plots. Conversely, negative values correspond with a decreaseing trend. This is consistent with the notion that positive coefficients in logsitic regression indicate a multiplicative increase in the odds of an outcome, while negative coefficients signify a decrease. 
...
```{r final-kfold-res, echo = F,warning = F, message= F}
...
rownames(final_acc_f1) <- c("Final Model")
...
```{r pca-cont2,  include=F, fig.width = 5, fig.height =5,warning = F}
# var <- get_pca_var(res.pca.wrist)
...
```{r pca-cont-combo2,  include=F, fig.width = 5, fig.height = 5,warning = F}
# var <- get_pca_var(res.pca)

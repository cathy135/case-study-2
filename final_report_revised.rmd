...
We then investigated the prediction accuracy and f1 scores of each logistic regression model using a 5-fold cross validation approach. We chose to conisder the f1 score, which is the harmonic mean of precision and recall, due to the fact that this metric is more appropriate for settings with class imbalance. In our current dataset, we see that the stressed response occurs more than 50% of the time, warranting a consideration of f1 in addition to accuracy. In order to ensure that certain subjects were not over or underrepresented in the testing and training sets, we created stratified folds in which each individual’s representation was proportional to that of their representation in the entire dataset. As indicated in Table 1, the combined data model was shown to be superior in both accuracy and f1 score. For the wrist-worn and combined data models, sensitivity analysis was conducted by variying both the number of principal components and the choice of window size. Results (___) indicate that our models are not strongly sensitive to either. 

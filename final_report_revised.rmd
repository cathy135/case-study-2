...
Sense_Wrist(PCAdim = 4, win_size = 5)
Sense_Wrist(PCAdim = 5, win_size = 5)
Sense_Wrist(PCAdim = 6, win_size = 5)
Sense_Wrist(PCAdim = 4, win_size = 10)
Sense_Wrist(PCAdim = 5, win_size = 10)
Sense_Wrist(PCAdim = 6, win_size = 10)
Sense_Wrist(PCAdim = 4, win_size = 15)
Sense_Wrist(PCAdim = 5, win_size = 15)
Sense_Wrist(PCAdim = 6, win_size = 15)

Sense_Combo(PCAdim = 4, win_size = 5)
Sense_Combo(PCAdim = 5, win_size = 5)
Sense_Combo(PCAdim = 6, win_size = 5)
Sense_Combo(PCAdim = 4, win_size = 10)
Sense_Combo(PCAdim = 5, win_size = 10)
Sense_Combo(PCAdim = 6, win_size = 10)
Sense_Combo(PCAdim = 4, win_size = 15)
Sense_Combo(PCAdim = 5, win_size = 15)
Sense_Combo(PCAdim = 6, win_size = 15)
```
...
# Benchmark: no pca, no interaction, all variables


```{r combo-wrist-kfold-cv-nosubj, include = F,warning = F}
set.seed(123)

kfoldcv_benchmark = function(df) {
  ...
  # run model
  noPCA_model = glm(Label ~ .-Subject, family = binomial(link="logit"), data=train)
  #summary(wrist.full)
  pred.test = noPCA_model %>% predict(test, type= "response")
  fitted_probs = c(fitted_probs, pred.test)
  pred.test = ifelse(pred.test>0.5, 1,0)
  predicted_values = c(predicted_values, pred.test)
  actual_values = c(actual_values, test$Label)
...
finalcvoutput = kfoldcv_benchmark(remove_select_cov)
```

```{r}
benchmark_acc_per_fold = finalcvoutput %>%
  group_by(fold_no) %>%
  summarize(accuracy = mean(actual_values==predicted_values))
benchmark_f1_per_fold = finalcvoutput %>%
  group_by(fold_no) %>%
  summarize(f1 = F1_Score(actual_values, predicted_values, positive = NULL)) 


benchmark_acc_f1_data <- as.matrix( c( round(mean(benchmark_acc_per_fold$accuracy),2),
                                    round(mean(benchmark_f1_per_fold$f1),2)  ),
                                    ncol=2,
                                    byrow=TRUE)
benchmark_acc_f1_data

colnames(benchmark_acc_f1_data)
colnames(benchmark_acc_f1_data) <- c("benchmark")
rownames(benchmark_acc_f1_data) <- c("Accuracy","F1")
benchmark_acc_f1_data <- as.table(benchmark_acc_f1_data)
kable(benchmark_acc_f1_data, caption = "Accuracy and F1 for Benchmark Only Data Models")
```
